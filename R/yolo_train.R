...
yolo_train <- function(model,
  ...
  # Accept both .yaml and .yml — try the other extension if not found
  if (!file.exists(data)) {
    alt <- if (grepl("\.yaml$", data, ignore.case = TRUE))
      sub("\.yaml$", ".yml",  data) else
      sub("\.yml$",  ".yaml", data)
    if (file.exists(alt)) {
      cli::cli_alert_info(glue("Using {alt} (.yml/.yaml variant auto-resolved)"))
      data <- alt
    } else {
      abort(glue("data file not found: {data}
Also tried: {alt}"))
    }
  }
